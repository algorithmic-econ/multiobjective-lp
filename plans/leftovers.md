...
- **SCOPE NARROWED mid-session by user directive** (saved to memory `multiobjective-lp-pb-validation-scope`): PB-shape validation lives ONLY on `ElectionSolver` (`election_solver.py::validate_election_program`, called from `ElectionSolver.actualSolve`) — covers `GreedySolver` + `PhragmenSolver` only. Explicitly NOT added to `muoblpsolvers/utils.py` (generic, shared by all 10 incl. non-PB `SummedObjectivesLpSolver`) and NOT bolted onto MES-family/`SingleTransferableVote`/`ExpandingApprovals`/`SolidCoalitionRefinement` (bare `LpSolver` + `mes/common.py::prepare_mes_parameters`, no shared PB-aware base). **These 6 solvers have ZERO shape validation after T13** — open follow-up: give them PB validation too, likely by first migrating them onto a shared `ElectionSolver`-like base (architecture change, not just bolting on a function call). Candidate owner: new ticket, or fold into T16 (feasibility unification) / T17 (coverage sweep).
...
- Bonus: closed all 3 `mes_constrains.py` pyright suppressions tagged "T13" in the T03/T05 leftover inventory (`reportOptionalOperand` on `constraint.value()`, lines 21/34/35) — added `_require_value()` helper that raises `PulpSolverError` on `None` (defensive; never hit at runtime since `setInitialValue` always runs first in that solver's loop). Confirmed via `pyright` still 0 errors after removing suppressions (real fix, not just moved).
