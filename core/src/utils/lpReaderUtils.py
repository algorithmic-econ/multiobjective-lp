from pulp import LpMaximize, LpMinimize, LpVariable, LpBinary, LpConstraintLE, LpConstraintGE, LpConstraint, \
    LpAffineExpression
...
def read_lp_file(filename) -> MultiObjectiveLpProblem:
    ...
    problem_data['sense'] = LpMaximize if lines[1].lower().startswith('maximize') else LpMinimize
    ...
    for constraint_line in lines[lines.index('Subject To\n') + 1:lines.index('Bounds\n')]:
        ...
        c_sense = LpConstraintLE if constraint_line[sign_idx:sign_idx + 2] == '<=' else LpConstraintGE
    ...
    for target_line in lines[lines.index('OBJECTIVES:\n') + 1:lines.index('END_OBJECTIVES:\n')]:
    ...
    problem = MultiObjectiveLpProblem(problem_data['name'], problem_data['sense'], problem_data['objectives'])
