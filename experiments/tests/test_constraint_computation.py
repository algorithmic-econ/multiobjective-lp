...
def test_district_lb_basic():
    ...
    inst = make_instance([p1, p2], 500)

    assert compute_district_lb(inst) == 200


def test_district_lb_single_project():
    p1 = make_project("p1", 400, set())
    inst = make_instance([p1], 1000)

    assert compute_district_lb(inst) == 600


def test_district_lb_budget_equals_max():
    p1 = make_project("p1", 300, set())
    inst = make_instance([p1], 300)

    assert compute_district_lb(inst) == 0


def test_district_lb_empty_instance():
    inst = make_instance([], 500)

    # max defaults to 0, so lb = 500
    assert compute_district_lb(inst) == 500
...
def test_shares_empty_profiles():
    ...
    inst = make_instance([p1], 1000)
    prof = make_approval_profile({}, {})

    result = compute_voter_category_shares(
        {"d1": inst}, {"d1": prof}, "APPROVAL", 1000, False
    )
...
def test_shares_single_voter_approval():
    ...
    by_name = {"p1": p1, "p2": p2}
    inst = make_instance([p1, p2], 1000)
    prof = make_approval_profile({"v1": ["p1", "p2"]}, by_name)
...
def test_shares_cost_weighting():
    ...
    p2 = make_project("p2", 200, {"env"})
    by_name = {"p1": p1, "p2": p2}
    inst = make_instance([p1, p2], 900)
    prof = make_approval_profile({"v1": ["p1", "p2"]}, by_name)

    result = compute_voter_category_shares(
        {"d1": inst}, {"d1": prof}, "APPROVAL", 900, True
    )
...
def test_shares_multi_cat_project_splits():
    ...
    by_name = {"p1": p1}
    inst = make_instance([p1], 1000)
    prof = make_approval_profile({"v1": ["p1"]}, by_name)
...
def test_shares_two_voters_different_prefs():
    ...
    by_name = {"p1": p1, "p2": p2}
    inst = make_instance([p1, p2], 1000)
    prof = make_approval_profile({"v1": ["p1"], "v2": ["p2"]}, by_name)
...
def test_category_lb_single_voter():
    ...
    result = compute_category_lb(
        "edu", {"d1": inst}, {"d1": prof}, "APPROVAL", 1000, False
    )
...
def test_category_lb_use_cost():
    ...
    result = compute_category_lb(
        "edu", {"d1": inst}, {"d1": prof}, "APPROVAL", 900, True
    )
...
def test_category_lb_missing_category():
    ...
    result = compute_category_lb(
        "sports", {"d1": inst}, {"d1": prof}, "APPROVAL", 1000, False
    )
