...
class LpElection:
    def __init__(
        ...
        voters_preferences: Dict[AgentId, List[AgentId]],
    ) -> None:
        (
            self.lp_problem,
            self.selected_candidates,
            self.voters_objectives,
    ...
    def get_infeasible_constraints(self) -> List[LpConstraint]:
        ...
        return [
            constraint
            for constraint in self.lp_problem.constraints.values()
            if (constraint.sense == LpConstraintGE and constraint.value() < 0)
            or (constraint.sense == LpConstraintLE and constraint.value() > 0)
        ]

    def define_constraint_ub(
        self, name: str, project_costs: Dict[AgentId, int], budget: int
    ):
        # Σ (selected[i] * cost[i]) <= budget
        constraint = LpConstraint(
            e=lpSum(
                self.selected_candidates[c_id] * c_cost
                for c_id, c_cost in project_costs.items()
            ),
    ...
    def define_lp_problem(
        ...
        selected_candidates = LpVariable.dicts(
            "selected_candidate", candidates, cat="Binary"
        )
        ...
        multi_objectives = {
            voter: LpElection.define_voter_objective(
                f"target_{voter}", approved_candidates, selected_candidates
            )
    ...
    def define_voter_objective(
        ...
        candidate_variables: Dict[AgentId, LpVariable],
    ) -> LpAffineExpression:
        approved_candidates_variables = [
            candidate_variables[candidate_id] for candidate_id in voter_preferences
        ]
        return LpAffineExpression(
            [[variable, 1] for variable in approved_candidates_variables], name=name
        )
